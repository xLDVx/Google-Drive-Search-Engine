...
class TestDownloadWorker:
    ...
    @patch('WorkerThreads.DownloadWorker.build')
    def test_file_download_error_handling(self, mock_build, mock_google_drive_service):
        """Test error handling during file download."""
        # Configure mock to raise an exception
        mock_google_drive_service.files().get_media.return_value.execute.side_effect = Exception("Download failed")
        mock_build.return_value = mock_google_drive_service
        
        worker = DownloadWorker()
        
        with pytest.raises(Exception, match="Download failed"):
            worker._download_file('test_file_id', 'test_file.txt')
